...
auto SelfDelete() -> int {
    ...
    if (!GetModuleFileNameW(nullptr, path, sizeof( path ) )) {
        BeaconPrintf(CALLBACK_OUTPUT, "GetModuleFileNameW Failed: %d\n", GetLastError());
        return EXIT_FAILURE;
    }
    ...
    if (h_file == INVALID_HANDLE_VALUE) {
        BeaconPrintf(CALLBACK_OUTPUT, "CreateFileW Failed: %d\n", GetLastError());
        return EXIT_FAILURE;
    }
    ...
    const auto p_rename = static_cast<PFILE_RENAME_INFO>(HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size_rename));
    if (!p_rename) {
        BeaconPrintf(CALLBACK_OUTPUT, "HeapAlloc Failed: %d\n", GetLastError());
        return EXIT_FAILURE;
    }
    ...
    if (!SetFileInformationByHandle(h_file, FileRenameInfo, p_rename, size_rename)) {
        BeaconPrintf(CALLBACK_OUTPUT, "SetFileInformationByHandle Failed: %d\n", GetLastError());
        return EXIT_FAILURE;
    }
